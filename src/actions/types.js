export const SET_ALERT = 'SET_ALERT'
export const REMOVE_ALERT = 'REMOVE_ALERT'

export const SET_TOGGLE_WP = 'SET_TOGGLE_WP'
export const SET_TOGGLE_AWP = 'SET_TOGGLE_AWP'
export const SET_TOGGLE_EWP = 'SET_TOGGLE_EWP'

export const REGISTER_SUCCESS = 'REGISTER_SUCCESS'
export const REGISTER_FAIL = 'REGISTER_FAIL'

export const LOGIN_SUCCESS = 'LOGIN_SUCCESS'
export const LOGIN_FAIL = 'LOGIN_FAIL'

export const ADMIN_LOGIN_SUCCESS = 'ADMIN_LOGIN_SUCCESS'
export const ADMIN_LOGIN_FAIL = 'ADMIN_LOGIN_FAIL'

export const LOAD_USER = 'LOAD_USER'
export const AUTH_ERROR = 'AUTH_ERROR'

export const GET_WORKPROFILE = 'GET_WORKPROFILE'
export const ADD_WORKPROFILE = 'ADD_WORKPROFILE'
export const EDIT_WORKPROFILE = 'EDIT_WORKPROFILE'
export const WORKPROFILE_ERROR = 'WORKPROFILE_ERROR'

export const GET_SKILLS = 'GET_SKILLS'
export const SKILLS_ERROR = 'SKILLS_ERROR'

export const GET_TECHNOLOGIES = 'GET_TECHNOLOGIES'
export const TECHNOLOGIES_ERROR = 'TECHNOLOGIES_ERROR'

export const LOGOUT = 'LOGOUT'
export const ADMIN_LOGOUT = 'ADMIN_LOGOUT'

export const SET_JOB = 'SET_JOB'
export const GET_JOBS = 'GET_JOBS'
export const JOBS_ERROR = 'JOBS_ERROR'

export const GET_MYJOBS = 'GET_MYJOBS'
export const CLEAR_MYJOBS = 'CLEAR_MYJOBS'
export const MYJOBS_ERROR = 'MYJOBS_ERROR'

export const GET_BIDS = 'GET_BIDS'
export const BIDS_ERROR = 'BIDS_ERROR'

// export const GET_SERVICES = 'GET_SERVICES'
// export const SERVICES_ERROR = 'SERVICES_ERROR'

// export const GET_SERVICE = 'GET_SERVICE'
// export const SERVICE_ERROR = 'SERVICE_ERROR'

// export const GET_CATEGORIES = 'GET_CATEGORIES'
// export const CATEGORIES_ERROR = 'CATEGORIES_ERROR'

// export const GET_REVIEW = 'GET_REVIEW'
// export const REVIEW_ERROR = 'REVIEW_ERROR'

// export const GET_USERS = 'GET_USERS'
// export const USERS_ERROR = 'USERS_ERROR'

// export const GET_BOOKINGS = 'GET_BOOKINGS'
// export const BOOKINGS_ERROR = 'BOOKINGS_ERROR'

// export const GET_REQUESTS = 'GET_REQUESTS'
// export const REQUESTS_ERROR = 'REQUESTS_ERROR'

// export const GET_REVIEWS = 'GET_REVIEWS'
// export const REVIEWS_ERROR = 'REVIEWS_ERROR'

// export const SET_USER = 'SET_USER'
// export const SET_CATEGORY = 'SET_CATEGORY'
// export const SET_SERVICE = 'SET_CATEGORY'
// export const SET_BOOKING = 'SET_BOOKING'
